...
int main() {
    ...
    const char* cola2_open_session = "\x02\x02\x02\x02" // STX
                                     "\x00\x00\x00\x0D"  // Length: 13 bytes
                                     "\x00\x00"          // Hub and NoC
                                     "\x00\x00\x00\x00"  // SessionID
                                     "\x00\x01"          // ReqID
                                     "\x4F\x58"          // Cmd = "O", Mode = "X"
                                     "\x1E"              // Timeout = 30 sec
                                     "\x00\x00";         // ClientID Length = 0, ClientID Flexstring = 0

    send(sock, cola2_open_session, 21, 0);
    ...
    memcpy(&session_id, &response[8], sizeof(session_id));

    // Step 3: Call LocInitializeAtPose
    const char* initialize_pose = "\x02\x02\x02\x02"  // STX
                                  "\x00\x00\x00\x2C"  // Length: 44 bytes
                                  "\x00\x00"          // Hub and NoC
                                  "\x2D\x6C\x27\x33"  // SessionID (from response)
                                  "\x00\x03"          // ReqID
                                  ...
                                  "\x03\xE8";         // Search radius: 1 m = 1 000 mm

    send(sock, initialize_pose, 56, 0);
    ...
    const char* cola2_close_session = "\x02\x02\x02\x02"  // STX
                                      "\x00\x00\x00\x0A"  // Length: 10 bytes
                                      "\x00\x00"          // Hub and NoC
                                      "\x2D\x6C\x27\x33"  // SessionID
                                      "\x00\x05"          // ReqID
                                      "\x43\x58";         // Cmd = "C", Mode = "X"

    send(sock, cola2_close_session, 18, 0);
